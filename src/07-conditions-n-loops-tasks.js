...
function doRectanglesOverlap(rect1, rect2) {
  ...
  if (rect1.left + rect1.width < rect2.left || rect2.left + rect2.width < rect1.left) return false;
  return true;
}
...
function findFirstSingleChar(str) {
  ...
  str.split(' ').join('').split('').forEach((x) => {
    if (!arrUnique.includes(x) && !arrRepeat.includes(x)) {
      arrUnique.push(x);
    } else if (arrUnique.includes(x)) {
      arrUnique = arrUnique.filter((y) => y !== x); arrRepeat.push(x);
    }
  });
  if (arrUnique.length > 0) return arrUnique[0];
  return null;
}
...
function isCreditCardNumber(ccn) {
  const result = ccn.toString()
    ...
    .reverse()
    .map((item, index) => {
      if (index % 2 !== 0) return item * 2 > 9 ? item * 2 - 9 : item * 2;
      return item;
    })
...
function isBracketsBalanced(str) {
  ...
  function check(arr) {
    ...
    for (let i = 0; i < arr.length - 1; i += 1) {
      if (brackets.includes(arr[i] + arr[i + 1])) { arrCopy[i] = 'br'; arrCopy[i + 1] = 'br'; }
    }

    const arrNew = arrCopy.filter((x) => x !== 'br');

    if (arrCopy.length === arrNew.length) return false;
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 1; i < pathes.length; i += 1) {
    const itemArr = pathes[i].split('');
    let { length } = itemArr;
    if (itemArr.length > result.length) length = result.length;
    for (let j = 0; j < length; j += 1) {
      if (result[j] !== itemArr[j]) result = result.slice(0, j);
    }
  }
...
function getMatrixProduct(m1, m2) {
  ...
  //   const m2RowsCount = m2.length;
  const m2ColsCount = m2[0].length;
  const result = new Array(m1RowsCount); // initialize array of rows
  for (let row = 0; row < m1RowsCount; row += 1) {
    result[row] = new Array(m2ColsCount); // initialize the current row
    for (let col = 0; col < m2ColsCount; col += 1) {
      result[row][col] = 0; // initialize the current cell
